...
void autonomous() {
	...
	for (int i = 0; i <= instr.size(); i++) { // for each instruction in the instr variable
		...
		int left = atoi(strtok(current, &delimiter)); // get left
		int right = atoi(strtok(NULL, &delimiter)); // get right
		...
		uint current_time = pros::millis();
		if (current_time < time) {
			pros::delay(time - current_time);
		}
	}
}
