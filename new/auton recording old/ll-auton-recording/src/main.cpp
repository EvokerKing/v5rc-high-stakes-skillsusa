...
void initialize() {
	...
	file = fopen("/usd/recording3.txt", "w"); // open the recording file with write mode
	if (file == nullptr) {
...
void opcontrol() {
	...
	while (pros::millis() <= 60000) { // main loop
		...
		output += std::to_string(leftThumbstick) + ":" + std::to_string(rightThumbstick) + ":" + std::to_string(pros::millis());
		output += buttons;
